...
// Define render_smart_pdf on window to make it globally accessible
window.render_smart_pdf = function(frm, print_format_name) {
	...
	// Open a new blank window/tab immediately to prevent popup blockers
	...
	// Write dynamic wrapper page to load PDF and auto-trigger browser printing
...
// Form refresh hook for "Generate Smart PDF" button under Print menu
			...
			if (r.message && r.message.length > 0) {
				...
				frm.add_custom_button(
					...
					function () {
						...
						// Pre-populate with the first available template if any
...
// Monkey-patching PrintView class prototype to override standard Print screen
function hook_print_view_class(PrintViewClass) {
	if (!PrintViewClass || !PrintViewClass.prototype) return;
	if (PrintViewClass.prototype._smart_print_hooked) return;
	...
	// 1. Override setup_sidebar to add the Advanced Print Format selector
	...
	// Override refresh_print_format to clear Advanced Format when standard format changes
	...
	// 2. Override show to check for formats and pre-populate
			...
			callback: function (r) {
				if (r.message && r.message.length > 0) {
					if (me.advance_print_format_control) {
						...
						// We do NOT pre-populate with Advanced Print Format anymore. 
						// It defaults to standard Print Format.
					}
				}
			}
		});
	...
	// 3. Override preview to show PDF iframe
	...
	PrintViewClass.prototype.preview = function() {
		...
		if (advance_format) {
			...
			$print_format.off("load.smart_preview").on("load.smart_preview", function() {
				...
				setTimeout(() => clearInterval(checkPagination), 10000);
			});
	...
	// 4. Override render_pdf to download PDF directly
	...
	// 5. Override render_page to route custom prints
...
// Watch frappe.ui.form namespace for PrintView class assignment
...
// Fallback: Trigger hook on route/page change events in case of dynamic execution
