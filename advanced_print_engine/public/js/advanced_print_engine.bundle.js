...
function ape_fetch_linked_advanced_format(print_format) {
	...
	return frappe
		.xcall("advanced_print_engine.api.generate_pdf.get_linked_advanced_format", {
			print_format,
		})
...
function ape_load_smart_preview(print_view, advance_format, preview_id) {
	...
	ape_fetch_advanced_format_meta(advance_format).then((meta) => {
		...
		frappe
			.xcall("advanced_print_engine.api.generate_pdf.get_smart_preview_html", {
...
function ape_reset_standard_preview_container(print_view) {
	...
	// Clear smart-preview HTML so Frappe can inject standard preview cleanly
	try {
		const iframe = $print_format[0];
		...
		// ignore cross-origin or empty document errors
	}
...
function hook_print_view_class(PrintViewClass) {
	...
	PrintViewClass.prototype.setup_sidebar = function () {
		...
		// print.js only stores $input — keep the full control for reliable get_value()
		...
		this.advance_print_format_control = this.add_sidebar_item({
			...
			change: () => {
				if (this._is_clearing_fields || this._ape_linked_from_print_format) return;
	...
	PrintViewClass.prototype._ape_unlock_advance_print_format = function () {
		if (!this.advance_print_format_control) return;
		this._ape_linked_from_print_format = false;
	...
	PrintViewClass.prototype._ape_sync_linked_advanced_format = function () {
		...
		// Clear immediately when print format is removed — don't wait for async API
		if (!print_format) {
	...
	PrintViewClass.prototype._ape_resolve_advance_format = function () {
		...
		// A standard Print Format is selected — only use advanced engine if explicitly linked
		if (print_format) {
		...
		// No Print Format selected — allow manual Advanced Print Format picker
